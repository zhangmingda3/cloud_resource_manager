...
def manager_ecs(token,url_project,sub_project_id):
    ...
    for ecs in ecs_list:
        ...
        user_id = ecs['user_id']
        ecs_name = ecs['name']
        if id not in protected.ECS and time.time() > SHUTOFF_time and ecs_status != "SHUTOFF":
            SHUTOFF_list.append({"id":id})
            logger.logger('%s  ECS_name:%s  id:%s Will be Shut Off.\n' % (time.strftime("%Y-%m-%d %H:%M:%S"),ecs_name,id))
        if id not in protected.ECS and time.time() > DELETE_time:
            DELETE_list.append({'id':id})
            logger.logger('%s  ECS_name:%s  id:%s Will be DELETE!.\n' % (time.strftime("%Y-%m-%d %H:%M:%S"), ecs_name, id))
    if len(SHUTOFF_list) >0:
        poweroff_results = poweroff.poweoff_ecs(token, url_project, sub_project_id, SHUTOFF_list)
        logger.logger('%s  ECS Shut Off result:%s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S"), poweroff_results))
    if len(DELETE_list) >0:
        delete_results = delete.del_ecs(token,url_project,sub_project_id,DELETE_list)
        logger.logger('%s  ECS Delete result:%s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S"),  delete_results))
